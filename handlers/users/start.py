...
#         [
#             InlineKeyboardButton(text="➕ A'zo bo'lish", url='https://t.me/Code_Mates_Academy'),
#             InlineKeyboardButton(text="✅ Tasdiqlash", callback_data='Sms')
#         ]
#     ])
...
# async def check_user(func):
#     async def wrapper_func(message):
#         chat_id = message.from_user.id
#         data = await dp.bot.get_chat_member(chat_id=-1001480779812,user_id=chat_id)
...
async def bot_start(message: types.Message):
    ...
    data = await dp.bot.get_chat_member(chat_id=-1001480779812,user_id=message.from_user.id)
    if data.status in ["left","kicked"]:
        ink_menu = InlineKeyboardMarkup(row_width=2, inline_keyboard=[
            [
                InlineKeyboardButton(text="➕ A'zo bo'lish", url='https://t.me/Code_Mates_Academy'),
...
async def start_button(call:types.CallbackQuery):
    ...
    if query == "check_user":
        ...
        if data.status in ["left","kicked"]:
            ...
            await dp.bot.send_message(chat_id=chat_id,text="Nima gap")
